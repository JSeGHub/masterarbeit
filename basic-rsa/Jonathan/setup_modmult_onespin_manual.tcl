...
###################################
    ...
    proc generate_wcet_signal_properties {script_path output_file module_name input_names read_in ready_out clock_name reset_name mpwid_name mpwid filtered_interrupts granularity} {
    ...
    foreach signal_name $input_names {
        ...
        puts $fd "    t ##0 ($module_name.$read_in == 1'b1) and"
        puts $fd "    t ##0 ($module_name.$ready_out == 1'b1) and"
        puts $fd "    t ##0 ($module_name.$signal_name\[$mpwid-1:$mpwid-$granularity-1\] == INPUT_A\[a\]) and"
...
#####################
    ...
    generate_wcet_signal_properties $script_path "property_checker_generated.sva"  $module_name $input_names $read_in $ready_out $clock_name $reset_name $mpwid_name $mpwid $filtered_interrupts $granularity
