...
if {$opti_type == "esp"} {
...
} elseif {$opti_type == "lzp"} {
    set bit_len [expr {1 << $granularity}]
    if { $input_operation_names != "" && $operation_flag == 1 } {
        set op_count 0
        foreach operation $input_operation_names {
            # get length of opertion
            ...
            for {set l 0} {$l < $op_num} {incr l} {
                ...
                for {set i 0} {$i < [llength $input_data_names]} {incr i} {
                    if { $secure_data_input_zero($i,$l) == 1} {
                        puts $fp "            if ([lindex $input_data_names $i]\_label == 0) begin"
                        puts $fp "              case ([lindex $input_data_names $i]\[$width_name-1:$width_name-[expr {$bit_len+1}]-$granularity\])"

                        for {set j 0} {$j < $bit_len} {incr j} {
                            set pattern ""
                            for {set k 0} {$k < $bit_len} {incr k} {
                                if {$k == $j} {
                                    append pattern "1"
                                } elseif {$k > $j} {
                                    append pattern "x"
                            ...
                            puts $fp "          ${bit_len}'b$pattern: t_array\[$i\] = $data_max_zero($i,$j,0);"
                        }
        ...
        for {set i 0} {$i < [llength $input_data_names]} {incr i} {
            if {$secure_data_input_zero($i,0) == 1} {
                puts $fp "      if ([lindex $input_data_names $i]_label == 0) begin"
                puts $fp "        case ([lindex $input_data_names $i]\[$width_name-1:$width_name-[expr {$bit_len+1}]\])"
    ...
    if { $input_operation_names != "" && $operation_flag == 1 } {
        ...
        #        puts $fp "        case ([lindex $input_data_names $i]\[$width_name-1:$width_name-[expr {$bit_len+1}]\])"
        #        for {set j 0} {$j < $bit_len} {incr j} {
        #            set a_val_bin [format "%0${bit_len}b" [expr {1 << $j}]]
        #            puts $fp "          $bit_len'b$a_val_bin: t_array\[$i\] = $data_max($i,[expr {$bit_len -1 - $j}],0);"
        #        }
        #        puts $fp "          default: t_array\[$i\] = $T_WCET_EXE;"
        #        puts $fp "        endcase"
        #        puts $fp "      end else t_array\[$i\] = 0;\n"
        ...
        for {set i 0} {$i < [llength $input_data_names]} {incr i} {
            if {$secure_data_input_zero($i,0) == 1} {
                puts $fp "      if ([lindex $input_data_names $i]_label == 0) begin"
                puts $fp "        case ([lindex $input_data_names $i]\[$width_name-1:$width_name-$coarse_bits\])"

                # Coarse case: 3'b000 — apply detailed leading-zero logic
                puts $fp "          ${coarse_bits}'b000: begin"
                puts $fp "            casez ([lindex $input_data_names $i]\[$width_name-1:$width_name-[expr {$bit_len+1}]\])"
                for {set j 0} {$j < $bit_len} {incr j} {
                    set pattern ""
                    for {set k 0} {$k < $bit_len} {incr k} {
                        if {$k == $j} {
                ...
                # Other coarse cases — simplified handling
                puts $fp "          ${coarse_bits}'b001: t_array\[$i\] = 7;"
                puts $fp "          ${coarse_bits}'b010, ${coarse_bits}'b011: t_array\[$i\] = 8;"
                puts $fp "          default: t_array\[$i\] = 9;"
