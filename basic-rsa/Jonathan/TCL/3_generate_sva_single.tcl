...
# Function to search for the timestamp based on signal name and value
    ...
    set fd [open $script_path/$sva_file\_short.sva w]
...
########## test for leading zero
    if { $opti_type == "lzp" || $opti_type == "all"} {
        if { $input_operation_names != "" && $operation_flag == 1 } {
                ...
                for {set i 0} {$i < $op_num} {incr i} {
                    ...
                    foreach signal_name $input_data_names {
                        for {set j 0} {$j < (1 << $granularity)} {incr j} {
                            puts $fd "  // Property for $module_name.$signal_name"
                            puts $fd "  property $signal_name\_$i\_$j\_p(a, ts);"
                            # puts $fd "    t ##$timestamp \($module_name.$operation\[[lindex $input_operation_length 0]\] == $i\) and"
                            if { $granularity != 0} {
                                set timestamp [get_timestamp $ready_sequence "$valid_in" "1"]
                                puts $fd "    t ##$timestamp ($module_name.$signal_name\[$width_name-1:$width_name-[expr {1+$j}]] == [expr {1+$j}]'b1) and"
                                puts $fd "    t ##$timestamp ($module_name.$operation\[[lindex $input_operation_length 0]\] == $op_root'b$a_val_bin) and"
                            }
                            puts $fd "$seq \[*ts+1\]"
                            puts $fd "  implies "
                            ...
                            puts $fd ""
                            puts $fd "  generate"
                            puts $fd "      for (a = 0; a < \$size(INPUT); a++) begin: $signal_name\_$i\_$j\_zero"
                            puts $fd "         for (ts = T_BCET_IN; ts < T_WCET_IN; ts++) begin: $signal_name\_$i\_$j\_zero"
                            ...
                            puts $fd "            $signal_name\_$i\_$j\_p_a: assert property (disable iff ($interrupt_condition) $signal_name\_$i\_$j\_p (a,ts));"
                            puts $fd "         end"
                            puts $fd "      end"
                            puts $fd "  endgenerate"
                            puts $fd ""
            ...
            foreach signal_name $input_data_names {
                for {set i 0} {$i < (1 << $granularity)} {incr i} {
                    puts $fd "  // Property for $module_name.$signal_name\_$i\_zero"
                    puts $fd "  property ${signal_name}_$i\_zero_p(ts);"
                    if { $granularity != 0} {
                        set timestamp [get_timestamp $ready_sequence "$valid_in" "1"]
                        puts -nonewline $fd "    t ##$timestamp ($module_name.$signal_name\[$width_name-1:$width_name-[expr {1+$i}]] == [expr {1+$i}]'b"
                    ...
                    puts $fd ""
                    puts $fd "  generate"
                    puts $fd "      for (ts = T_BCET_IN; ts < T_WCET_IN; ts++) begin: $signal_name\_$i\_zero"
                    ...
                    puts $fd "         ${signal_name}_$i\_zero_p_a: assert property (disable iff ($interrupt_condition) ${signal_name}_$i\_zero_p (ts));"
                    puts $fd "      end"
                    puts $fd "  endgenerate"
                    puts $fd ""
                }
...
######### test leading zero end
    ...
    puts "Output file: $sva_file\_short.sva"
